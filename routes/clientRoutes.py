...
async def get_clients(id_cliente: str | None = None):
    if id_cliente:
        if Service.get_client_by_id_service(id_cliente) == None:
            raise HTTPException(
                status_code=404, detail=f"Cliente con id 'id_cliente'='{id_cliente}' non trovato")
    else:
        if Service.get_all_clients_service() == []:
            raise HTTPException(status_code=404, detail=f"Nessun Cliente trovato")
    try:
        if id_cliente:
...
async def get_assignments_details(id_commessa: str):
    if Service.get_assignment_details(id_commessa) == None:
        raise HTTPException(
            status_code=404, detail=f"Cliente con id 'id_commessa'='{id_commessa}' non trovato")
    try:
...
async def delete_client(id_cliente: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=e.msg)
    raise HTTPException(status_code=201, detail="Cliente eliminato con successo")
...
async def add_client(client: Client):
    try:
        ...
        raise HTTPException(status_code=500, detail=e.msg)
    raise HTTPException(status_code=201, detail="Cliente aggiunto con successo")
...
async def update_client(id_cliente: str, new_data: Client):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=e.msg)
    raise HTTPException(status_code=201, detail="Cliente aggiornato con successo")
